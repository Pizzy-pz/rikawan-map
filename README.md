...
Googleマップに載っていない店、雑居ビルの奥にある店、入口がわかりにくい店などの情報は今までは口頭か個人のメモに頼っており
新人が店舗を探しにくい、またそもそも知らないという事が起きていました。それを解決するために作りました。
...
工夫点は、以下の３点です。
①APIエラーの対応としてレスポンスが返らないケースを想定したエラーハンドリングの実装
②プロキシ処理としてバリデーション、レート制限、認証チェックを搭載
③住所を座標方式にすることでGoogle検索で出てこない店舗へ対応
大変だったこと
...
## 設計
[サイトマップ](./設計/サイトマップ.jpg) / [ER図](./設計/ER図.png) / [API設計書](./設計/API設計書.png) / [インフラ構成図](./設計/インフラ構成図.png)
